...
Rubyで良く目にする`[1, 2, 3].map(&:to_s)`のような処理について書いておきたい。まず以下はすべて同じ結果となる。
...
というのもブロック付きメソッドを呼び出す際に`to_proc`メソッドを持つオブジェクトは`&`修飾して引数に渡すことができるからである。このことはリファレンスに書かれている。
...
`Symbol#to_proc`の実装は以下のようになっているらしい。これを見ると冒頭のイディオムも理解しやすいのではないだろうか。`Symbol#to_proc`は何かのオブジェクトを引数にとって、何かのオブジェクトに対してシンボル名のメソッドを呼び出す`Proc`オブジェクトを返すのだ。
...
つまり`()`内の`&:to_s`は`&:to_s.to_proc`と同じであり、この`Proc`オブジェクトの処理内容は`{|obj, &args| obj.send :to_s, *args}`であるかのように振る舞うのである。これでこのイディオムが`[1, 2, 3].map{|n| n.to_s}`と同じ結果になることが理解できる。
